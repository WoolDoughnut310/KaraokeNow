...
function provideOrganizeImports(response, documents) {
    if (!response || response.body.length === 0) {
        return [];
    }
    // Return a single code action with potentially multiple edits.
    return [
        lsp.CodeAction.create('Organize imports', { documentChanges: response.body.map(edit => (0, protocol_translation_1.toTextDocumentEdit)(edit, documents)) }, types_1.CodeActionKind.SourceOrganizeImportsTs.value)
    ];
}
